    ...
    while (1) {
        ...
        switch (bc[pFrame->pc]) {
            ...
            case iaload: // 46 /*(0x2e)*/Load int from array
                ...
                pFrame->stack[pFrame->sp - 1] = pObjectHeap->GetObjectPointer(pFrame->stack[pFrame->sp - 1].object)[
                        pFrame->stack[pFrame->sp].intValue + 1];
                pFrame->sp--;
                pFrame->pc++;
                break;
            case aaload: //50
            ...
            case aastore: // 83 - both seems same (TODO: Check it)
                pObjectHeap->GetObjectPointer(pFrame->stack[pFrame->sp - 2].object)[
                        pFrame->stack[pFrame->sp - 1].intValue + 1] = pFrame->stack[pFrame->sp];
                pFrame->sp -= 3;
                pFrame->pc++;
                break;
            ...
            case iadd: //96
                ...
                break;
            case ladd: // 97 /*(0x61)*/
                longVal = (i8) (((i8) pFrame->stack[pFrame->sp - 3].intValue << 32) |
                                (i8) pFrame->stack[pFrame->sp - 2].intValue) +
                          (i8) (((i8) pFrame->stack[pFrame->sp - 1].intValue << 32) |
                                (i8) pFrame->stack[pFrame->sp].intValue);
...
/*
https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-5.html#jvms-5.4.3.6
...
To resolve an unresolved symbolic reference to a call site specifier involves three steps:
   ...
   * A call site specifier gives a symbolic reference to a method handle which is to serve as the bootstrap method for a
    dynamic call site. The method handle is resolved (§5.4.3.5) to obtain a reference to an instance of
    java.lang.invoke.MethodHandle.

   * A call site specifier gives a method descriptor, TD. A reference to an instance of java.lang.invoke.MethodType is
    obtained as if by resolution of a symbolic reference to a method type (§5.4.3.5) with the same parameter and return
    types as TD.

   * A call site specifier gives zero or more static arguments, which communicate application-specific metadata to the
    bootstrap method. Any static arguments which are symbolic references to classes, method handles, or method types are
    resolved, as if by invocation of the ldc instruction (§ldc), to obtain references to Class objects,
    java.lang.invoke.MethodHandle objects, and java.lang.invoke.MethodType objects respectively. Any static arguments
    that are string literals are used to obtain references to String objects.
...
** During resolution of the symbolic reference to the method handle in the call site specifier, or resolution of the
 symbolic reference to the method type for the method descriptor in the call site specifier, or resolution of a symbolic
 reference to any static argument, any of the exceptions pertaining to method type or method handle resolution
 (§5.4.3.5) may be thrown. **
 */
...
void ExecutionEngine::ExecuteInvokeDynamic(Frame *pFrameStack) {
    ...
    /*
		1. Load the bootstrap class - DONE
		2. Create object of bootstrap class (or not for static invocation) - DONE
		3. Find the bootstrap method - DONE
        3a. Find dynamic invocation type (static/virtual/..) DONE
	...
	*/
    ...
    method.access_flags = pFrameStack[1].pMethod->access_flags;
    ...
    //static
    if (handle_ref_kind == REF_invokeStatic) {
        params--;
    }
...
pNativeMethod GetNativeMethod(std::string strSign) {
    if (false) {
            ...
            "java/lang/invoke/StringConcatFactory@makeConcatWithConstants(Ljava/lang/String;J)Ljava/lang/String;")) {
